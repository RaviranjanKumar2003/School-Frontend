...
export function Sidenav({ brandImg, routes }) {
  ...
  const [profileImage, setProfileImage] = useState(brandImg);

  const validButtonColors = [
    "white",
    "blue-gray",
    "gray",
    "brown",
  ...
  const safeSidenavColor = validButtonColors.includes(sidenavColor)
    ? sidenavColor
    : "blue";

  const accountantAvatar =
  ...
  useEffect(() => {
    ...
    if (normalizedRole === "hod") {
      ...
      const accountantData = getParsed("accountant");

      data = Array.isArray(accountantData)
        ? accountantData.find(
            (a) => String(a.id) === String(userId)
          )
        : accountantData;
      ...
      setProfileImage(
        data?.profileImage || accountantAvatar
      );
    }
  ...
  return (
    <>
      ...
      <aside
        className={`
          ${sidenavTypes[sidenavType] || sidenavTypes.white}
          ${
            openSidenav
              ? "translate-x-0"
              : "-translate-x-80"
