...
public class EmailService {
    ...
    // =========================================
    // 🔥 NEW METHOD (FEE REMINDER)
    // =========================================
    public void sendFeeReminder(String toEmail, String studentName, Double pendingAmount) {

        SimpleMailMessage message = new SimpleMailMessage();

        message.setTo(toEmail);
        message.setSubject("⚠️ Fee Payment Reminder");

        message.setText(
                "Dear " + studentName + ",\n\n" +
                        "Your pending fee amount is ₹" + pendingAmount + ".\n" +
                        "Please pay your fee as soon as possible.\n\n" +
                        "Note: Late payment may cause additional dues.\n\n" +
                        "Regards,\nSchool Administration"
        );

        mailSender.send(message);

        logger.info("Fee reminder sent to: {}", toEmail);
    }
