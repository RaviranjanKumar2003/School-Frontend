...
export function Home() {
  ...
  fetch(`http://localhost:8080/api/results/student/${id}`)
  .then((res) => res.json())
  .then((data) => {
  ...
  const failed = latestResults.some(
    (r) =>
      r.status === "FAIL" ||
      r.status === "ABSENT"
  );
  ...
  fetch(`http://localhost:8080/api/notifications/student/${id}`)
   ...
   fetch(`http://localhost:8080/api/subjects/school/${data.schoolId}/class/${data.classId}`)
   .then((res) => res.json())
   .then((subjectsData) => {

    // ✅ GET TEACHERS
    fetch(`http://localhost:8080/api/professors/by-school/${data.schoolId}`)
      .then((res) => res.json())
      .then((teacherData) => {
